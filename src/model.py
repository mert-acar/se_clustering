    ...
    def __init__(self, filters: List[int], in_ch: int = 3) -> None:
        ...
        for in_c, out_ch in zip(channels[:-1], channels[1:]):
            encoder_layers.append(
                nn.Conv2d(in_c, out_ch, kernel_size=3, stride=2, padding=1)
            )
        ...
        for in_c, out_ch in zip(channels[:-1], channels[1:]):
            decoder_layers.append(
                nn.ConvTranspose2d(
                    in_c, out_ch, kernel_size=3, stride=2, padding=1, output_padding=1
                )
            )
...
class SelfExpression(nn.Module):
    def __init__(self, n: int):
        super().__init__()
        self.c = nn.Parameter(
            1.0e-10 * torch.ones(n, n, dtype=torch.float32), requires_grad=True
        )
...
if __name__ == "__main__":
    ...
    assert z.shape == torch.Size(
        [bs, filters[-1] * (h // (2 ** len(filters))) * (w // (2 ** len(filters)))]
    ), "Latent code shape do not match with expectation!"
    assert zc.shape == torch.Size(
        [bs, filters[-1] * (h // (2 ** len(filters))) * (w // (2 ** len(filters)))]
