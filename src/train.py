...
from utils import get_device, create_dir, closest_factors
...
def plot_scores(scores: Dict[str, List[float]], out_path: Optional[str] = None):
    ...
    fig, axs = plt.subplots(
        r, c, tight_layout=True, figsize=(5 * c, 5 * r), squeeze=False
    )
...
def train_ae(config: Dict[str, Any], verbose: bool = True):
    ...
    model = AutoEncoder(
        filters=config["model"]["filters"], in_ch=dataset_info["in_ch"]
    ).to(device)
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, **config["train"]["scheduler_args"]
    )
    ...
    for epoch in range(config["train"]["num_epochs"]):
        ...
        if epoch - best_epoch >= config["train"]["early_stop"]:
            if verbose:
                print(
                    f"No improvements in {config['train']['early_stop']} epochs, stop!"
                )
            break
...
def train_cluster(config: Dict[str, Any], verbose: bool = True):
    ...
    config["data"]["batch_size"] = (
        config["data"]["sample_per_class"] * dataset_info["num_classes"]
    )
    ...
    for epoch in range(config["train"]["num_epochs"]):
        ...
        if epoch % 5 == 0 or epoch == config["train"]["num_epochs"] - 1:
            ...
            try:
                ...
                cluster_scores["silhouette_score"] = float(
                    silhouette_score(z.detach().cpu().numpy(), y)
                )
