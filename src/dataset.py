...
DATASET_INFO = {
    "COIL20": {"num_classes": 20, "in_ch": 1},
    "CIFAR10": {"num_classes": 10, "in_ch": 3},
    "MNIST": {"num_classes": 10, "in_ch": 1},
}
...
class COIL20(Dataset):
    def __init__(
        ...
        self.filelist = [
            ...
            for f in os.listdir(os.path.join(data_root, split))
            if f.endswith(".png")
        ]
    ...
    def targets(self) -> List[int]:
        return list(
            map(
                lambda x: int(x.split("/")[-1].split("__")[0].replace("obj", "")),
                self.filelist,
            )
        )
...
def get_dataset(
    dataset_name: str, split: str = "test", sample_per_class: Optional[int] = None
) -> Dataset:
...
def get_dataloader(
    ...
    return DataLoader(
        dataset_cls, batch_size=batch_size, num_workers=num_workers, shuffle=isTrain
    )
